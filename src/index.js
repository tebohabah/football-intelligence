...
function predict(f){
  ...
  const lh=Math.max(
    .2,
    f.homeXg*(1+.08*ratingEdge+.05*formEdge)
  );

  const la=Math.max(
    .2,
    f.awayXg*(1-.06*ratingEdge-.03*formEdge)
  );
  ...
  const confidence=Math.round(
    clamp(
      ...
      Math.abs(probs.home-probs.away)*28,
      0,
      97
    )
  );
...
function liveBaselinePrediction(f){
  ...
  return {
    ...
    providerKickoffUtc:f.kickoffUtc,
...
async function sync(env){
  ...
  const [a,b]=await Promise.all([
    ...
    fetchProvider(
      `https://v3.football.api-sports.io/fixtures?date=${todayISO(1)}`,
      env
    )
  ]);
...
async function safeReadFixtures(env){
